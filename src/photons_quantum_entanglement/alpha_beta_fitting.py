...
@click.option(
    "-a", "--alpha", type=clickp.IntListParamType(), default="0,45,90,135"
)
@click.option(
    "-b", "--betac", type=clickp.FloatListParamType(), default="3,3,3,3"
)
@click.option(
...
def fit_alpha_beta(excel_path, alpha, betac, output_dir, sheet_name):
    ...
    for alpha_value, betac_value in zip(alpha, betac):
        ...
        # Results json
        with open(output_dir / f"alpha_beta_fit_{alpha_value}_results.json", mode="w") as fd:
            json.dump(
                dict(
                    result=popt.tolist(),
        ...
        # Plot
        plt.title(fr"Coincidence rate by angle, $\alpha={alpha_value}$")
        plt.xlabel(fr"$\beta$ [degrees]")
        plt.ylabel("Coincidence count")
        plt.plot(df["beta"], fitted_values * max_column)
        plt.scatter(df["beta"], df[data_column])
        plt.savefig(output_dir / f"alpha_beta_fit_{alpha_value}")
        plt.clf()

        # Residuals Plot
        plt.title(fr"Coincidence rate by angle residuals, $\alpha={alpha_value}$. $\chi^2={chi2:.2f}$")
        plt.xlabel(fr"$\beta$ [degrees]")
        plt.ylabel("Coincidence count error")
        plt.errorbar(df["beta"], np.zeros(shape=np.shape(x)), yerr=residuals * max_column)
